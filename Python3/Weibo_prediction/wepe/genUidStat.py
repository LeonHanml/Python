...
def loadData():
    traindata = pd.read_csv("D:\\www\\data\\Weibo Data\\Weibo Data\\weibo_train_data(new)/weibo_train_data.txt",header=None,sep='\t')
    traindata.columns = ["uid","mid","date","forward","comment","like","content"]

    testdata = pd.read_csv("D:\\www\\data\\Weibo Data\\Weibo Data\\weibo_predict_data(new)/weibo_predict_data.txt",header=None,sep='\t')
