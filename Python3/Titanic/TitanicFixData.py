...
def set_missing_ages(df):
    ...
    # fit到RandomForestRegressor之中 调整n_estimators随机森林中树的数量默认10个树，精度递增显著
...
data_test = pd.read_csv("D:\\www\\data\\Titanic\\test.csv")
...
data_test['PassengerId'].value.reshape(-1, 1)
predictions = predictions.reshape(-1, 1)
result = pd.DataFrame({'PassengerId': data_test['PassengerId'].as_matrix(), 'Survived': predictions.astype(np.int32)})
result.to_csv("'D:\\www\\data\\Titanic\\logistic_regression_predictions.csv", index=False)
test['Fare_scaled'].value.reshape(-1, 1)
test['Age_scaled'].value.reshape(-1, 1)
test.to_csv("'D:\\www\\data\\Titanic\\test_data.csv", index=False)
